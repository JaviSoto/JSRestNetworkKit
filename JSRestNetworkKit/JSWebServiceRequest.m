...
#define WebServiceRequestDetailDebug 0
    ...
    #define WebServiceRequestDebugLog(s,...) NSLog(@"%@", [NSString stringWithFormat:@"%@ %@", NSStringFromClass([self class]), [NSString stringWithFormat:s, ##__VA_ARGS__]])
#else
    #define WebServiceRequestDebugLog(s,...)
#endif

#if WebServiceRequestDetailDebug
    #define WebServiceRequestDebugDetailLog(s,...) NSLog(@"%@", [NSString stringWithFormat:@"%@ %@", NSStringFromClass([self class]), [NSString stringWithFormat:s, ##__VA_ARGS__]])
#else
                                                ...
                                                failure:(void (^)(NSURLRequest *request, NSURLResponse *response, NSError *error))failure
{
    ...
    switch (requestType) {
        ...
        default:
        {
            request = [[JSBaseWebServiceProxy instance] requestWithMethod:@"GET" path:self.url parameters:[parameters parametersDictionary]];
            break;
        } 
        ...
        {
            request = [[JSBaseWebServiceProxy instance] requestWithMethod:@"POST" path:self.url parameters:[parameters parametersDictionary]];
            break;
        }
    }
                      ...
                      failure:(void (^)(NSURLRequest *request, NSURLResponse *response, NSError *error))failure
{
    ...
    if (operation)
    {
        ...
        [[JSBaseWebServiceProxy instance].operationQueue addOperation:operation];
    }
}
...
{    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSMutableURLRequest *request = [[JSBaseWebServiceProxy instance] multipartFormRequestWithMethod:@"POST" path:self.url parameters:[parameters parametersDictionary] constructingBodyWithBlock: ^(id formData) {
